...
class ScouterAttention(nn.Module):
    ...
    def forward(self, inputs_pe, inputs, loc, loc2, mask_size, weight=None, things=None):
        ...
        if self.vis:
            ...
            vis(slots_vis_raw, loc, loc2, mask_size, self.args.feature_size, weight, things)
        return updates, attn


def vis(slots_vis_raw, loc, loc2, mask_size, size, weight=None, things=None):
    ...
    for i in range(b):
        ...
        if weight is not None:
            ...
            for id, image in enumerate(slots_vis):
                ...
                image = Image.fromarray(image, mode='L').resize([mask_size, mask_size], resample=Image.BILINEAR)
                if things is not None:
                    order, category, cpt_num = things
                    loc_2 = f"{loc2}/cpt{cpt_num}/{category}/"
                    if id == cpt_num:
                        image.save(loc_2 + f'mask_{order}_{category}.png')
                        break
                    else:
                        continue
