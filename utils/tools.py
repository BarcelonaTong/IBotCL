...
def predict_hash_code(args, model, data_loader, device):
    ...
    for batch_idx, (data, label) in enumerate(data_loader):
        ...
        if is_start:
            ...
            all_pred_labels = pred_labels.unsqueeze(-1).cpu().detach().float()  
            is_start = False
        else:
            ...
            all_pred_labels = torch.cat((all_pred_labels, pred_labels.unsqueeze(-1).cpu().detach().float()), 0)  

    return all_output.numpy().astype("float32"), all_label.numpy().astype("float32"), all_pred_labels.numpy().astype("float32"), round(accs / L, 4)
