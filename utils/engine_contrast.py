...
def test_MAP(args, model, database_loader, test_loader, device):
    ...
    if args.num_classes > 400:
        ...
        database_hash, database_labels, database_preds, database_acc = predict_hash_code(args, model, database_loader, device)
    print('Waiting for generate the hash code from test set')
    test_hash, test_labels, test_preds, test_acc = predict_hash_code(args, model, test_loader, device)
    print('Calculate MAP.....')
